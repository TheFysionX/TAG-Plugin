...
| Claude Code | First-party `claude auth status --json` plan observation | Project JSONL terminal assistant usage snapshots | Subscription type is plan-only evidence. Journal detail remains opt-in; partial rows without `message.stop_reason` are withheld. |
...
| Gemini / Antigravity | Explicitly consented sanitized `statusLine` wrapper | Prospective status-line usage only | No historical backfill. The reversible wrapper preserves and forwards the prior status line, records an allowlisted local capture, and can report plan/quota-window observations. |
| Grok Build | Local session summaries | None | Activity diagnostics only. It never reconstructs billable tokens or uploads accounting events. |
...
Antigravity is also opt-in. Its wrapper is installed only during confirmed installation when Gemini/Antigravity is authorized. It does not replace another product silently: it saves the prior `statusLine`, forwards stdin to it, and restores it on removal when TAG still owns the entry. Plan observations travel only in signed heartbeat fields. A quota reset is recorded only after its observed reset boundary advances; a changed percentage alone is not a reset. Grok Build is never a token source, and DeepSeek `api_payg` is never inferred from a hosted-model record.
