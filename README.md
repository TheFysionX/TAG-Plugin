...
| --- | --- | --- | --- |
| Codex | Official `codex app-server` `account/usage/read` aggregate | Rollout JSONL `token_count` records | Official aggregate is a provider-backed checkpoint. Journal model detail is explicit opt-in. |
| Claude Code | A future documented aggregate API | Project JSONL terminal assistant usage snapshots | Journal model detail is explicit opt-in. Partial rows without `message.stop_reason` are withheld. An undocumented `stats-cache.json` is detected but never used to invent model detail. |
| Kimi Code | A future content-free aggregate API | v0.28 `wire.jsonl` `usage.record` records | Version-pinned, explicit opt-in support. The current backend advertises Kimi and accepts its registered models/modes. |
