...
| Provider-plan inflation | Capability-scoped plan observations: Codex app-server; current first-party Claude auth plus organization-bound provider process/account-bootstrap `rate_limit_tier`; Antigravity's version-pinned same-user loopback account-status RPC with a pinned local TLS certificate and native effective-tier allowlist; or explicit DeepSeek API evidence | Antigravity SQLite usage, legacy `planInfo`, a stale cache from another account, hosted model name, conversation string, journal record, or local Grok summary cannot fabricate a retail entitlement. Antigravity Starter stays explicitly ambiguous between Free and AI Plus. |
