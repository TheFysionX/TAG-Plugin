...
| Provider-plan inflation | Capability-scoped plan observations: Codex app-server; current first-party Claude auth plus organization-bound provider process/account-bootstrap `rate_limit_tier` parsed at the exact account-object path; same-account non-Max/logout/account-switch revocation; optional consented Antigravity CLI status line; or explicit DeepSeek API evidence | Antigravity desktop SQLite metadata never produces a plan claim. A stale cache from another account, hosted model name, conversation string, journal record, or local Grok summary cannot fabricate or indefinitely preserve an entitlement. |
