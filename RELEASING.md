...
2. Run:
   ...
   node scripts/validate-release-contract.mjs v0.1.0
   ```
...
6. Download the published asset and verify it independently:
   ...
   ```text
   gh release download v0.1.0 -R TheFysionX/TAG-Plugin
   gh attestation verify tag-plugin-0.1.0.tgz -R TheFysionX/TAG-Plugin
   ```
...
7. Configure the deployment for The Artificial Games from the published release, never from a local build:
   ...
   CONNECTOR_RELEASE_TAG=v0.1.0
   CONNECTOR_RELEASE_COMMIT=<full 40-character tag commit SHA>
   CONNECTOR_RELEASE_ASSET=tag-plugin-0.1.0.tgz
