...
The preferred source is content-free, provider-backed usage data. Today that is available for Codex through the official `codex app-server` `account/usage/read` RPC. It returns lifetime/daily aggregates but no model or fast-mode breakdown. The same official app-server can expose a coarse account plan and quota-window boundaries. Claude's `claude auth status --json` may expose a subscription type. Those plan/window values are bounded heartbeat metadata, not usage events. The connector does not read Codex auth files; the official subprocess may use its existing signed-in session and controls any provider refresh it performs.
...
Antigravity is a separate, prospective opt-in. The confirmed installer may set a reversible `statusLine` wrapper only after the user authorizes Gemini/Antigravity. The wrapper receives the status-line JSON, retains only an allowlisted sanitized capture (plan tier, model/mode/token counters, and quota-window boundaries), then forwards the original stdin to the saved prior status line. It never uploads the raw status-line input. It restores the saved setting on uninstall only when TAG still owns that setting. Grok Build local summaries are read only as activity diagnostics and never turn into token accounting events. DeepSeek `api_payg` is only observed from explicit API evidence; no hosted model record creates it.

On first connection, journal processing covers all retained history that can be discovered and parsed within the bounded file and line safety limits. The server applies scoring eligibility separately; the connector does not discard Raw Tokens merely because they are old. Usage is deduplicated by account-scoped source ID and reduced locally into stable host-service/source-session/UTC-hour/raw-model/raw-mode/surface aggregates before upload; prompt, response, tool, command, filename, and repository content never participates in that aggregation. The outbound `serviceProviderId` identifies the authorized host service for consent, plan, reset, checkpoint, Raw-total, and provider-filter boundaries. The separate outbound `provider` identifies the actual model vendor for model validation and Kleos. The canonical model sent for scoring is not part of aggregate identity. Codex, Claude, and Kimi each keep an independent settled-through watermark. Disabled, unavailable, truncated, or event-limited providers retain their prior watermarks. Malformed or oversized records produce explicit partial coverage and a privacy-safe loss count; the connector may advance past that completed scan range so new valid records continue syncing, but it does not claim the historical repair is complete. Later runs normally process only newly settled hours. A changed partial replay of a finalized session-hour reuses its stable event ID rather than uploading a separately scoreable aggregate.
...
- Last sync/heartbeat times, content-free adapter health counts, persistent provider-level partial-coverage counts when unreadable records prevent a complete repair claim, and pending/acknowledged plan or actual quota-window reset observations. Antigravity wrapper state stores the former `statusLine` value and a capture fingerprint; the sanitized capture is stored separately under the Antigravity home.
...
Heartbeat requests contain only observed time, `healthy`/`degraded`/`paused`, connector version, the prior request digest, and—when Codex is connected—an optional content-free local generation ID plus snapshot digest (or explicit null). They may also contain bounded provider plan observations and actual quota-window reset observations: provider, service surface, recognized plan code, observation time, evidence type, and for a reset the old/new window boundary and key. A quota percentage moving down does not create a reset; the prior observed boundary must advance. On an exact match the server returns only a compact current acknowledgement; on mismatch or reinstall it can return the content-free Codex generation ID, snapshot digest, lifetime total, and date-to-token map already held for the same connected account. An incomplete legacy raw-only backfill is one reason for local `degraded` status; its provider names and counts are not sent in the heartbeat.
