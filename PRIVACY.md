...
Codex provider-backed checkpoints contain only provider, source, UTC observation period, total tokens, and deterministic checkpoint ID. A distinct lifetime-authority checkpoint is sent first, followed by daily checkpoints. They are used for reconciliation and are not scored as model usage.
