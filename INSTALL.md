...
Codex plan and quota-window observations come from its official local app-server; Claude plan observation comes from `claude auth status --json`. These are content-free heartbeat metadata, not model usage. If Gemini/Antigravity is both allowed and explicitly selected, confirmed installation adds TAG's reversible status-line wrapper: it saves the existing `statusLine`, forwards the same input to it, records only a sanitized prospective capture, and restores it on uninstall unless the user later changes the setting. Grok Build summaries are diagnostic only and never produce token events. DeepSeek API plan evidence is accepted only when explicit API evidence is available; a hosted DeepSeek model does not imply it.
